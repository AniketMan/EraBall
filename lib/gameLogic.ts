...
function calcAstFactor(entries: { pr: PlayerRating; minScale: number }[]): number {
  ...
  return Math.max(0.90, Math.min(1.10, raw))
}
...
export function simulateSeason(
  ...
  const shooterCount           = entries.reduce((s, e) => { const w = STARTER_SLOTS.includes(e.pr.slot) ? 1.08 : 0.90; const f = e.pr.player.FG3_PCT ?? 0; return s + (f >= 0.40 ? e.minScale * 1.25 : f >= 0.37 ? e.minScale * 1.12 : f >= 0.34 ? e.minScale : f >= 0.30 ? e.minScale * 0.5 : f >= 0.25 ? e.minScale * 0.25 : 0) * w }, 0)
